...
(function(){
  ...
  function mount(){
    ...
    fab.onclick=function(){ ov.style.display="flex";document.body.classList.add("pcv3-open");
    ...
    var x=ov.querySelector(".x"); if(x) x.onclick=function(){ ov.style.display="none";document.body.classList.remove("pcv3-open"); };
    ov.addEventListener("click",function(e){ if(e.target===ov) ov.style.display="none";document.body.classList.remove("pcv3-open"); });
    syncGate();
  ...
  function boot(){
    ...
    /* 2026-09-09 owner: "핀보드 없애고 인스타처럼 그냥 피드로" — the board view is gone from the tabs; the feed is
       one column, newest first, no tilt. Lost & found still uses the board layout underneath. */
    let view="feed", filter="all", dense=false, sortDesc=true, query="";
    let items=[];
    function syncItems(){
    ...
    function render(){
      ...
      document.getElementById("s-mode").textContent=view==="feed"?"Feed · newest first":"Lost & found";
    ...
    const boardCols=()=>view==="feed" ? 1 : Math.max(1,Math.floor((feed.clientWidth+GAP)/pitch(boardW())));
